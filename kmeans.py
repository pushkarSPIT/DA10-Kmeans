	...
	while num_done < k:
		...
		for data_point in data:  # Checking if the random number lies in a
			...
			rand_num -= point_distances[
				data_point]  # This step is needed as this interval is done and we need to update the value for the next interval
		num_done += 1
	# TODO [task3]:
...
if __name__ == '__main__':
    ...
    for experiment in range(args['numexperiments']):
        ...
        print
        'Convergence achieved: {}'.format(hasconverged(all_means[-1], all_means[-2]))
    ...
    print
    'Average number of iterations: {}'.format(float(totaliter) / args['numexperiments'])
